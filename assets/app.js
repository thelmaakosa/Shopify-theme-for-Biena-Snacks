$(function() {
  ...
  if ($(window).width() >= 768) {
    ...
    $("#featured_links").waypoint(function() {
      $(this).find(".column, .columns").addClass("animated ");
    }, { offset: '80%' });
...
//     });  
  ...
  var productImageSlider = $('.product_slider');
  productImageSlider.flexslider({
    ...
    start: function() {
    	productImageSlider.removeClass('loading');
  	}
  });
  ...
  $('#search-toggle, .search-close').on('click', function(e) {
    $('.search-close, .search-submit').css('padding-top', $('.header').height() + 10 + 'px')
  ...
  if ($(window).width() >= 959) {
   ...
   $('.mm-search input').keypress(function(e) {
    if(e.which == 13) {
      window.location = '/search?type=product&q=' + $(this).val();
    }
  });
    ...
    if ($cartBtn && $cartBtn.size()) {
      ...
      if (cart.item_count == 0) {
        ...
        cart_items_html += '<li class="mm-label">' +
          ...
          cart_items_html += '<input type="submit" name="checkout" class="action_button right" value="Checkout" />' +
            '<a href="javascript:void(0)" class="action_button edit_cart right">Continue Shopping</a>' +
          '</li>';
  ...
  selectCallback = function(variant, selector) {
    ...
    if (variant && variant.available == true) {
      ...
      var message = variant ? "Out of Stock Online!" : "Unavailable";
      $('.was_price', $product).text('');
